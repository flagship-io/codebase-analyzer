...
func TestCustomRegex(t *testing.T) {
	AddCustomRegexes(`[{"file_extension":".tsx?","custom_regexes":["\\s*['\"](.+?)['\"]"]}]`)
	...
	assert.Equal(t, 1, len(foundLanguageRegex.CustomRegexes))
	assert.Equal(t, "\\s*['\"](.+?)['\"]", foundLanguageRegex.CustomRegexes[0])
}
